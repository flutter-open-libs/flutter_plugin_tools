          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                if (assetPath.isNotEmpty)
                  SizedBox(width: gap),
                Text(title,style: titleTextStyle ?? const TextStyle(color: Color(0xFF131732), fontSize: 16.0)),
                SizedBox(width: gap),
                if(showSub)
                  Expanded(
                    child: KeyboardListener(
                      ...
                      child: TextField(
                        ...
                        style: subTitleTextStyle ?? const TextStyle(color: Color(0xFF131732), fontSize: 14.0),
                        decoration: InputDecoration(
                          prefixText: "  ",
                          isDense: true,
                          contentPadding: subTitleContentPadding ?? const EdgeInsets.symmetric(vertical: 12.0),
                          counterText: "", // 去掉底部计数
                          ...
                          hintStyle: subTitleHintTextStyle ?? const TextStyle(color: Colors.grey, fontSize: 14.0),
                        ),
                      ),
                    ),
                  ),
                ...
                if (showNext)
                  InkWell(
                    onTap: onSubTap ?? onTap ?? () => debugPrint('SingCellItem sub arrow'),
                    child: nextIcon ?? const Icon(Icons.arrow_forward_ios,size: 16.0,color:Colors.grey),
                  ),
              ]),
            ),
          ),
