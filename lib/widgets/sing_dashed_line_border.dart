...
/// 虚线边框
class SingDashedLineBorder extends BoxBorder {
  final double dottedLength;
  final double dottedSpace;

  const SingDashedLineBorder({
  ...
  const SingDashedLineBorder.fromBorderSide(
    BorderSide side, {
    this.dottedLength = 5,
    this.dottedSpace = 3,
  })  : top = side,
  ...
  const SingDashedLineBorder.symmetric({
  ...
  factory SingDashedLineBorder.all({
    ...
    return SingDashedLineBorder.fromBorderSide(side, dottedLength: dottedLength, dottedSpace: dottedSpace);
  }

  static SingDashedLineBorder merge(SingDashedLineBorder a, SingDashedLineBorder b) {
    assert(BorderSide.canMerge(a.top, b.top));
    assert(BorderSide.canMerge(a.right, b.right));
    assert(BorderSide.canMerge(a.bottom, b.bottom));
    assert(BorderSide.canMerge(a.left, b.left));
    return SingDashedLineBorder(
            ...
            case BoxShape.rectangle:
              if (borderRadius != null) {
                ...
                if (width == 0.0) {
                  ...
                  //
                  canvas.drawPath(_buildDashPath(Path()..addRRect(inner), dottedLength, dottedSpace),
                      paint
                        ..isAntiAlias = true
                        ..style = PaintingStyle.stroke
                        ..strokeWidth = width);
              ...
              // print('rect');
              canvas.drawPath(_buildDashPath(Path()..addRect(rect.deflate(width / 2.0)), dottedLength, dottedSpace), paint);
              break;
          }
          return;
      }
  ...
  BoxBorder? add(ShapeBorder other, {bool reversed = false}) {
    if (other is SingDashedLineBorder &&
      ...
      return SingDashedLineBorder.merge(this, other);
    }
    return null;
  }
